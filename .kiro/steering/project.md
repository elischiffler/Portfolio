...
- **Deploy**: GitHub Pages via GitHub Actions (`deploy.yml` — builds with Vite, deploys to `elischiffler.github.io/Portfolio`)
...
| Work        | Two work entries (Robert Half — Summer 2026 · Present, Sandia National Laboratories — Summer 2025), each with a clickable Crossfade that opens a lightbox image viewer. Robert Half shows 6 screenshots (3 Protiviti verification portal + 3 Robert Half verification portal); Sandia shows a single laser optics photo. Has a bottom snap anchor for scroll-up entry. Lightbox supports keyboard navigation (Escape, arrows). |
